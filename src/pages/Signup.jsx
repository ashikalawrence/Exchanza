...
import { Mail, Lock, User, GraduationCap, Heart, ArrowRight, Loader2, Leaf } from 'lucide-react';

const Signup = () => {
  const [formData, setFormData] = useState({
    fullName: '', email: '', password: '', collegeName: '', interests: ''
  });
  ...
  const fields = [
    { label: 'Full Name',                name: 'fullName',    type: 'text',     icon: <User size={17} />,           placeholder: 'John Doe',                          span: 2, required: true  },
    { label: 'Email Address',            name: 'email',       type: 'email',    icon: <Mail size={17} />,           placeholder: 'you@example.com',                   span: 1, required: true  },
    { label: 'Password',                 name: 'password',    type: 'password', icon: <Lock size={17} />,           placeholder: '••••••••',                          span: 1, required: true  },
    { label: 'College / University',     name: 'collegeName', type: 'text',     icon: <GraduationCap size={17} />,  placeholder: 'e.g. Stanford University',           span: 2, required: true  },
    { label: 'Favourite Book Interests', name: 'interests',   type: 'text',     icon: <Heart size={17} />,          placeholder: 'Science Fiction, Biography…',       span: 2, required: false },
  ];
    ...
    <div className="min-h-screen bg-hero-gradient relative flex items-center justify-center overflow-hidden py-16 px-4">
      ...
      <div className="relative z-10 w-full max-w-xl">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-bold text-[#263326] mb-2 tracking-tight">Join Exchanza</h1>
          <p className="text-[#7A8C7A] font-light">Start your book exchange journey today — it's free!</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-[0_8px_40px_rgba(79,111,82,0.1)] border border-[#E9E3D5]">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {fields.map(({ label, name, type, icon, placeholder, span, required }) => (
                <div key={name} className={span === 2 ? 'md:col-span-2' : ''}>
                  <label className="block text-sm font-semibold text-[#4F6F52] mb-1.5">{label}</label>
          ...
          <div className="mt-6 pt-6 border-t border-[#E9E3D5] text-center">
            <p className="text-[#7A8C7A] text-sm font-light">
              ...
              <Link to="/login" className="font-semibold text-[#4F6F52] hover:text-[#7BAE7F] transition-colors">
                Log in here
              </Link>
            </p>
          </div>
